...
public class Board {
    ...
    public static ArrayList<Num> merge(Num a, Num b,boolean backUp) {
        if (Math.abs(b.value - a.value) != 0) {
            return new ArrayList<Num>(Arrays.asList(a, b));
        } else {
            if (a.value != 0 || b.value != 0) {
                scoreBoard.addScore(a.value,backUp);
            }
    ...
    public static ArrayList<Num> revMerge(Num a, Num b,boolean backUp) {
        if (Math.abs(b.value - a.value) != 0) {
    ...
    public static Num[][] actLeft(Num[][] nn,boolean backUp) {
        for (int k = 3; k >= 0; k--) {
            Num[][] fixed1 = moveLeft(k, nn);
            for (int p = 0; p <= 2; p++) {
                fixed = revMerge(fixed1[k][p], fixed1[k][p + 1],backUp);
    ...
    public static Num[][] actRight(Num[][] nn,boolean backUp) {
        for (int k = 3; k >= 0; k--) {
            Num[][] fixed1 = moveRight(k, nn);
            for (int p = 2; p >= 0; p--) {
                fixed = merge(nn[k][p], nn[k][p + 1],backUp);
                nn[k][p] = fixed.get(0);
    ...
    public static Num[][] actUp(Num[][] nn,boolean backUp) {
        for (int k = 3; k >= 0; k--) {
            Num[][] fixed1 = moveUp(k, nn);
            for (int p = 0; p <= 2; p++) {
                fixed = revMerge(nn[p][k], nn[p + 1][k],backUp);
    ...
    public static Num[][] actDown(Num[][] nn,boolean backUp) {
        for (int k = 3; k >= 0; k--) {
            Num[][] fixed1 = moveDown(k, nn);
            for (int p = 2; p >= 0; p--) {
                fixed = merge(nn[p][k], nn[p + 1][k],backUp);
                nn[p][k] = fixed.get(0);
